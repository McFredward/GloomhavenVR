...
## Native item-tooltip smoke follow-up

A complete source audit of the new tooltip found `UIItemTooltip.Show -> ItemCardUI.Show -> UpdateState -> ItemCardEffects.ToggleEffect` also enables the original `fx_Smoke` emitter. The native item shader transitions themselves are explicitly 0.001 seconds (`ItemCardEffects.BurnCardTimeline` line 281 and `GhostOutOnTimeline` line 377), so inventing the multi-second ability-card burn would be wrong. Smoke has its own actual native lifetime: hiding/recycling the native tooltip makes its hierarchy inactive, while pooling resets `lastState` for the next native show.

The card lane's additive record56 extends the existing native plume stream with a bonus-slot namespace and the existing record45 slot identity. It samples the actual owner's original ItemCardUI emitters and reuses `RemoteCardPlume`; original prefab modules, seed, age, emission and board-relative pose determine the output. The widget stage continues stripping its particle controllers/systems, preventing a duplicate emitter. The shared `RemoteUseBarTooltip.IsShown` gate drives both tooltip and its sampled smoke. Selection coverage changes the front artwork only; it does not hide a visible tooltip's original smoke. Ordinary card-plume visibility policy is unchanged.
