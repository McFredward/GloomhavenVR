> # ARCHIVE — superseded 2026-09-08 by `.planning/STATE.md`
>
> **Do not act on this file.** It was the live handover until 2026-08-28 and its "current
> position" is **ModBuild 315**; HEAD is 483. Its §2 gate list (five gates, 1441 wire assertions,
> "six pre-existing warnings are OK") and its §3 record census (ids 1-32 in use, 37+ free) are
> both wrong now. Its §5 successor queue is from the ModBuild 104-149 era.
>
> It is KEPT, not deleted, because §4 is the only narrative record of rounds 1-315 and several of
> its entries are the reason a failed approach is not re-tried. Read it as archaeology. For
> anything current: `.planning/STATE.md`, then the build notes in `Net/NetProtocol.cs`.
