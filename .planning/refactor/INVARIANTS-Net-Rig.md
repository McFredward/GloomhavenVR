...
| Initiative track — the widget's ENTRY SET, the portraits, the numbers, the ENEMY block's order and the reorder animation's timing/geometry | `RemoteInitiativeTrack` → `RemoteWidgetMirror` over `InitiativeTrack.Instance` | a live **CLONE** of the game's own track, driven per frame from the original. Fallback (track absent): the mod-drawn chip strip over `InitiativeTrack.actorsUI` in sibling order, then `ScenarioManager.Scenario.AllAliveActors` deduped by `CActor.Class` and sorted by initiative with unknowns last |
| Initiative track — GRAYSCALE, the DEAD state, the EXTRA-TURN animation and the DAMAGE-WARNING animation | `RemoteWidgetMirror` (`Pair.Apply`, incl. `CopyMaterial`) | all four are plain replicated state and cost **zero wire**. `SetGrayscale` (InitiativeTrack.cs:614/625) is keyed on the phase + `Choreographer.m_CurrentActor`; `ShowExtraTurn` on `actor.IsTakingExtraTurn`; `ShowWarning` is raised/cleared inside the game's own replicated `PlayerSelectingToAvoidDamageOrNot` / `PlayerFinishedSelecting…` messages (Choreographer.cs:5479/5582), which **every** client processes. Grayscale and the GUIAnimator/UIFX effects ride a `Graphic.material`, which is why `Pair.CopyMaterial` had to exist before they could mirror at all |
| ⚠ **NOT global: the track's HOVER, its SELECTION frame, the mod's focus rings, the PLAYER block's on-screen ORDER and the selection-phase "still has to choose" RING** | `RemoteInitiativeTrack.ApplyHoverOverrides` (record 16), `ApplySelectionOverride` (**record 23**), `ApplyFocusRings` (record 22), `ApplyOrderOverride` (**record 27**) and `ApplySelectionGlow` (**record 27's owned mask + the replicated model**) | All three are per-PLAYER state that the clone copies from (or lays over) the LOCAL widget, so each must be stripped and re-decided from the peer's own synced value — otherwise MY mouseover / MY selected portrait appears on every peer's board. Vanilla's `InitiativeTrackActorAvatar.selectionObject` is client-local and never networked. **Record 22 is NOT the right source for it** (the ModBuild 84–86 mistake): the focus and the frame are different facts and come apart whenever an ENEMY or a foreign player is at turn (vanilla auto-selects `Choreographer.m_CurrentActor`, InitiativeTrack.cs:620) or a mod focus is live (`SelectionGuardPatches` suppresses vanilla's whole `OnClick`, so the owner's frame never moves). Record 23 carries the entries whose `selectionObject` was literally **active** on the owner's screen — no re-derivation left to be wrong, and enemies/objects included, which retires the old "enemy selections are not on the wire" limitation. **The ORDER was wrongly listed as GLOBAL until 2026-08-08** — for four builds, in this table and in `RemoteInitiativeTrack`'s own class doc. Vanilla's `InitiativeTrackActorBehaviour.CompareTo` (decompiled GH.Runtime, :160-171) sorts two PLAYER entries by `IsUnderMyControl` while `FFSNetwork.IsOnline && PhaseManager.CurrentPhase.Type == SelectAbilityCardsOrLongRest`, foreign first, so during card selection *my index 3 is your index 5* and a clone of the observer's widget showed the observer's arrangement on every peer's board. The mod had already written the fact down twice (`NetProtocol.ExtIdTrackHover`, `InitiativeHoverSampler`) — that is precisely why the hover and selection records name entries by stable id rather than by index — but nobody carried the order itself. Record 27 carries it as the sender's own sibling order, **not** a re-derivation: `CompareTo` is an *inconsistent* comparator (two foreign players compare 0 while each compares −1 against one of mine), so the outcome depends on `List<T>.Sort`'s introsort pivots and on the pre-sort input order, and re-deriving it would be the ModBuild-84 mistake again. The ENEMY block never permutes (the branch requires both sides to be player actors) and costs nothing. **The SELECTION-PHASE RING was in the same blind spot**: `WorldUI.Surfaces.InitiativeSelectionGlow` hangs a plain amber `Image` under each portrait and `Board.SelectionReadyHighlighter` fills it only for actors the LOCAL player controls (`IsUnderControlOrSingle()`), so `Instantiate` cloned it and `Pair.Apply` drove it — every peer's board wore the OBSERVER's ring set and never the owner's. It needs no record of its own: whose characters they are is record 27's owned mask (`IsUnderMyControl` is a local flag, CActor.cs:751), and whether one has COMMITTED is derived on the receiver from the replicated `CCharacterClass.RoundAbilityCards`/`LongRest` — the same list `RemoteControlBoard.SeatSlots` already draws a peer's card backs from, live through the selection phase via `ProxySetStartRoundDeckState`. Vanilla's `IsCardSelectionReady` reports "ready" for a foreign actor only because of its explicit `(!IsOnline \|\| IsUnderMyControl)` clause, which is a *visibility* gate over data that is present, not a data gap. **The REORDER SLIDE is genuinely GLOBAL** and was audited as per-viewer in error: `trackReorderDuration` is a serialized field of the one shared prefab, the trigger is `UpdateActors` off the replicated message stream, and the travel is holder-local uGUI px from the same layout group over the same entries at the same widths. `InitiativeReorderSlide`'s "its own tray orientation" refers to the panel's WORLD pose, which the mirror never copies (it re-fits into `PlayTray.InitiativeMountWidth` and `Pair.Apply` copies LOCAL rect state only). Its one per-viewer ingredient is which portrait STARTS in which slot — i.e. the order defect above — and the override stands down for the ~0.5 s the tween owns the row x, a stated limitation |
...
already public to peers is ever written; see `NetProtocol.ExtIdBoardTooltip`); the initiative
track's three per-viewer highlight families — hover (record 16), character focus (record 22) and
vanilla's SELECTION FRAME (record 23, the ids whose `selectionObject` is active on the owner's own
track; players, **enemies** and objects alike). (See the wire tables in Part I.)
...
| ~~The local player's own tuning offsets (`ConfirmUndoOffset`, `PinOffset`, `ItemUseSlotOffset`, `DecisionOffset`, `GenericButtonSpacing`, the per-board mesh pose, tuned `Fan*` config)~~ **(retired — extension record 28)** | the old entry drew every remote board at the **authored defaults** "so every remote board looks the same regardless of local tuning", which was the opposite of the 2026-08-08 1:1 ruling: a dial the owner has MOVED must read the same everywhere. They now ride record 28 **sparsely** — a field only when its quantized value differs from the shipped default for the sender's synced style, and no record at all when nothing differs, so an untuned player's packet is byte-identical to every previous build's. `RemoteBoardTuning` resolves them on the receiver against its own identical constants |
| Item **per-item face size** (the residual aspect after `CardWidth`, which record 28 now carries) | the receiver hosts the same game `ItemCardUI` widget through `RemotePileFronts` and can measure it; a per-card size list would cost ~25 B on every packet while a fan is open, for a number already on this client |
| `ClusterOffset` (the docked button-cluster mount POSITION) | not a tuning question: `ButtonCluster.AttachDocked` reads the mount's rotation and scale only, so mirroring the position would move the copy where the original never goes. Its SCALE is mirrored |
...
| ~~Empty-fan hint~~ **(retired — record 14, byte 1, bit 4)** | the observation that made it a DELIBERATELY-NOT is still exactly right and is now the REASON for the bit: the trigger is the palm-roll gate EDGE and the hand-card count cannot distinguish "gate opened with zero cards" from "fan closed", so it could never be inferred. It is therefore stated — one bit in record 14's reserved nibble, read off `Cards.EmptyFanHint.CurrentlyShown` (the renderer's own flag, so wire and picture cannot disagree). The plate's ANCHOR, FADE and TEXT stay derived on the receiver (`RemoteEmptyFanHint`) |
...
- **Why:** Adding a wire field for something already replicated wastes a scarce flag bit (**there are NONE left** — the rig byte, the extras byte and pile-browse byte A are all full, which is why every record since 22 has been a new TLV id instead: a TLV record costs no bit at all) and creates a second source of truth that can disagree with the game — which looks exactly like a desync. Conversely, rendering a VR-only fact "locally" shows the *local* player's value on the *peer's* board.
...
- **The two hazards, and how they are closed:**
  ...
  1b. *A branch the CALLER shows is not a branch the SOURCE shows* — and the drive's cost model assumed it was. `Pair.Apply` skipped any branch whose source node was `activeSelf == false` "because its interior is invisible", but `RemoteInitiativeTrack.ApplyHoverOverrides` shows the clone of an enemy's `MonsterBaseUI` info popup from the **peer's** hover (record 16) while this client's own copy of it is off. That branch's interior was therefore frozen at `Instantiate` time — and `MonsterBaseUI.GenerateCard` rebuilds the card body while the popup is INACTIVE, where **uGUI runs no layout**, so the snapshot carried caption rects nothing had ever resolved and TMP wrapped after every glyph (user report 2026-09-05 item 14, `remote-generinfo-textproblem.jpg`; identical shape to ModBuild 448's short-rest captions). It looked correct exactly when the LOCAL player had opened the same popup after the clone was built — which is the whole of "das tritt nicht immer auf". Such a branch is now declared at construction (`RemoteWidgetMirror`'s `externallyShownBranch` predicate → `Pair.External`): its `activeSelf` is never written here, its interior is never driven from an off source, and `Neutralize` **keeps the stock uGUI layout components inside it** (`LayoutGroup` / `ContentSizeFitter` / `LayoutElement`, assembly-restricted exactly as `IsStockLayout` already was) so the clone lays ITSELF out the moment the caller shows it. `RemoteInitiativeTrack.LayoutMirroredPopup` forces that rebuild once per clone and logs the measured widths / font sizes / line counts (`MIRRORED ENEMY INFO LAYOUT`). No wire byte: record 16 is 5 bytes of flags + actor id and carries no text at all.
