...
Additional review: element availability rings derive from the local augmentation-bar slots (`UIUseAugmentationsBar.RefreshAvailableElements`), while the original element creation/created GUIAnimators and looping FX write live target properties. A global model flag alone does not reproduce that owner presentation. Full native element snapshot work is in progress in this lane under record52; the historical GLOBAL classification does not excuse viewer-local rings or animation phases.
