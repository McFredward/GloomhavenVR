...
Owner VR late updates and appearance sampling reapply the floor after ordinary writers (integrator-owned call sites). Weak records belong to the original effects component/model, so detaching a VR wrapper cannot erase an ongoing original burn; recovery clears the record and destroying the original component releases its weak entry.
...
- Standalone harness compiles the actual production continuity/history/enumerator helpers and runs 25 runtime assertions, including first pre-reset observation, Lost membership before callback, native zero reset, recovery, active blue state, identity/actor/sender separation, exact yields, final raw jump to the cosmetic floor, cancellation and native exceptions.
