...
{
    private static readonly byte[] Golden = Hex.Bytes("51 35 01 44 33 22 11 40 30 20 10 00 00 48 41 00 00 20 41 00 00 00 3E 00 00 80 3E 01 00 00 20 41 00 00 00 3E 00 00 80 3E 01 00 00 20 41 00 00 00 3E 00 00 80 3E 01 80");
    private static readonly byte[] PacketGolden = Hex.Bytes("31 52 56 47 03 16 50 5E 01 44 33 22 11 40 30 20 10 00 00 48 41 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 51 35 01 44 33 22 11 40 30 20 10 00 00 48 41 00 00 20 41 00 00 00 3E 00 00 80 3E 01 00 00 20 41 00 00 00 3E 00 00 80 3E 01 00 00 20 41 00 00 00 3E 00 00 80 3E 01 80");
    public static void Run(Harness t)
    {
        var p = new TownActivityPose { WorkClock=10f,TransitionAge=.125f,FromBlend=.25f,Engaged=true };
        var state = new TownActivityState { Active=true,Epoch=0x11223344,Sequence=0x10203040,Clock=12.5f,Merchant=p,Temple=p,Enchantress=p,MerchantOfferingBlend=.5f };
        ...
        t.True(TownActivityCodec.Write(bytes,ref offset,in state)&&offset==55,"exact55byte additive record");
        for(int i=0;i<Golden.Length;i++)t.Equal(Golden[i],bytes[i],"literal byte"+i);
        t.True(TownActivityCodec.TryRead(Golden,2,53,out var read)&&read.Merchant.Engaged&&read.Temple.WorkClock==10&&read.MerchantOfferingBlend>0.49f&&read.MerchantOfferingBlend<0.51f,"literal vector reads");
        ...
        t.True(count==157&&bytes[5]==22&&TownActivityCodec.ReadPacket(bytes,count,out read,out _),"atomic157byte message22");
        ...
        for(int length=0;length<256;length++)if(length!=53)t.True(!TownActivityCodec.TryRead(Golden,2,length,out _),"record length"+length);
        for(int npc=0;npc<3;npc++)
        {
            var bad=(byte[])Golden.Clone();bad[15+npc*13+12]=2;t.True(!TownActivityCodec.TryRead(bad,2,53,out _),"reserved targetflag"+npc);
        ...
        for(int remaining=1;remaining<55;remaining++)
