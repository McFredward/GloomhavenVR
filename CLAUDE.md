...
3. **Wire:** magic `GVR1`, `Version` byte stays **3**, every change is an **additive TLV** record
   ...
   **Card identity never goes on the wire** — reveals go only through `Net/RevealGate.cs`.
   `scripts/wire-tests.sh` (213,722 assertions at ModBuild 485) is the proof; a `Write`+`TryRead` change
...
Current readings at 485: patch surface 107 classes / 165 methods · wire 213,722 assertions ·
