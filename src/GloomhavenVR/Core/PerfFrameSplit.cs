...
/// down.</para>
///
/// <para>COST. Two <see cref="Stopwatch.GetTimestamp"/> reads per frame for the logic span, three
...
internal static class PerfFrameSplit
{
    ...
    private static void AppendCameras(System.Text.StringBuilder sb)
    {
        ...
        sb.Append(" | per camera (main-thread, avg per frame it rendered — 'cull' is onPreCull→"
        ...
        for (int i = 0; i < Ranked.Count && i < 8; i++)
        {
            ...
            sb.Append(i == 0 ? " " : ", ").Append(c.Name).Append(' ')
              .Append((c.WindowSeconds * 1000d / frames).ToString("F2"))
              .Append("ms (cull ").Append((c.WindowCullSeconds * 1000d / frames).ToString("F2"))
              .Append(" + submit ").Append((c.WindowSubmitSeconds * 1000d / frames).ToString("F2"))
              .Append(") x").Append((c.WindowPasses / frames).ToString("F1"))
              .Append(" pass");
        }
    }
    ...
    private static string Verdict(float logic, float render, float blocked, float frame)
    {
        ...
        if (r >= 0.35f)
            return $"the RENDER LOOP owns the frame ({r * 100f:F0}%). The main thread is inside culling "
                   ...
                   + "listed above is a whole additional scene submission. WHICH HALF is in the per-camera "
                   + "cull/submit split above, and they have different levers — cull scales with how many "
                   + "renderers exist and pass the culling mask (see the SCENE line's per-layer counts), "
                   + "submit with how many draw calls the visible ones produce (materials, shadow passes, "
                   + "the forward depth prepass; see the SCENE and GFX lines).";
        if (b >= 0.5f)
    ...
    internal static string Describe() =>
        "frameSplit=ok (own clocks: logic span, render-loop span, per-camera cull/submit passes) "
