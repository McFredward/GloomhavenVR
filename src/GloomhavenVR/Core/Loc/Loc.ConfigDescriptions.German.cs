    ...
    private static Dictionary<string, string> BuildGerman() =>
        new(784, StringComparer.Ordinal)
        {
            ...
            ["Elements/EnvironmentResponse"] =
                ...
                + "— ein frisch entfachtes Element wirkt voll aufgeladen, ein schwindendes ist sichtbar "
                + "schwächer: weniger Frostflecken, schwächeres Licht, weniger von allem, sodass du auch "
                + "ohne Blick auf die Elementleiste siehst, dass es gleich erlischt. Nichts blinkt oder "
                + "pulsiert bei irgendeiner Stärke: die Reaktion blendet bei einem Wechsel über etwa eine "
                + "Sekunde ein und aus und steht danach völlig still. AUS entfernt die "
