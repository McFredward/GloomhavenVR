    ...
    private sealed partial class FadeDriver
    {
        ...
        private void EmitSolidBlockerLine()
        {
            ...
            VRLog.Note(Name,
                ...
                + $"all, so this is ADOPTION and {LeftoverMarkerCitation} carries the "
                + $"reject reason — BUT ONLY IF ITS FOOT IS ABOVE THE AIRBORNE BAR. That sweep's "
                + $"recorder returns without writing anything for a renderer whose AABB min.y is "
                + $"under floor+{MountedClearanceWU:0.0} wu (WallSegmentFade.Mounted.cs, "
                + $"NoteStructuralSkip), and the sweep's own bulk pre-filter drops such a "
                + $"candidate one screen earlier, also silently. So an [UNOWNED] piece STANDING ON "
                + $"THE FLOOR — which is what a pillar is — is named on no reject line anywhere, "
                + $"and the absence there is not evidence about it. For that population read the "
                + $"SPLIT PRE-FILTER line instead (ModBuild 386); (spelled with hyphens on "
                + $"purpose — a line that sends you to grep a marker must not itself match that "
                + $"grep); {_solidOwnerFading} [OWNER FADING, THIS PIECE UNWRITTEN] — the "
