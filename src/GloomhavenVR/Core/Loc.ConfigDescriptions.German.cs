    ...
    private static Dictionary<string, string> BuildGerman() =>
        new(784, StringComparer.Ordinal)
        {
            ...
            ["Water/OwnSurface"] =
                ...
                + "Zeichenreihenfolge, in der Farbe und mit den Wellen, die das Spiel selbst "
                + "gestaltet hat — was "
                + "sich ändert, ist der Shader dahinter: GloomhavenVR/WaterVR aus dem Bundle des "
                ...
                + "— er KANN also gar keine mitschwimmende Spiegelung erzeugen. WAS DAS WASSER "
                + "DES MODS IST: die Wellen-Normalkarte des Spiels, zweimal übereinander "
                + "gescrollt, mit den beiden Kachelungen und den beiden Geschwindigkeiten, die "
                + "das Spiel selbst gestaltet hat — das ist die gesamte Bewegung, die das echte "
                + "Wasser hatte. Das Funkeln entsteht aus diesen wandernden Wellen unter einer "
                + "FESTEN Lichtrichtung, niemals aus der Blickrichtung: ein Glanzlicht, das aus "
                + "der Position deines Auges berechnet wird, wandert beim Kopfdrehen über die "
                + "Fläche (genau das war der ursprüngliche Fehler) und sieht in jedem Auge anders "
                + "aus. UND DIE FLÄCHE HAT WIRKLICH RELIEF: die Grafikkarte unterteilt jedes "
                + "Wasserfeld im laufenden Bild feiner, und eine langsame Dünung bewegt die so "
                + "entstandenen Punkte tatsächlich — denn keine Schattierung der Welt macht eine "
                + "ebene Platte dreidimensional, und das Urteil über den Versuch lautete 'nur "
                + "weiße Streifen auf einer flachen Oberfläche'. ES IST STEHENDES WASSER, KEINE "
                + "STRÖMUNG: die Wellen heben und senken sich AN ORT UND STELLE, statt zu wandern "
                + "— vier davon, mit Wellenlängen ohne gemeinsames Maß, unter einer sehr langsamen "
                + "Modulation, die einen Teil des Tümpels fast still lässt. Genau das bedeutet "
                + "'eine Pfütze stehendes Wasser mit nur minimal Bewegungen', und genau deshalb "
                + "passiert auf jedem Feld etwas anderes. Feineinstellung über [Water] SwellHeight (wie hoch), [Water] "
                + "WaveScale (wie groß), [Water] RippleSpeed (wie schnell) und [Water] Shimmer "
                + "(wie viel Funkeln). Beckenbett "
            ...
            ["Water/RippleSpeed"] =
                "Wie schnell sich das Wasser bewegt, als Vielfaches der ausgelieferten Rate. EIN "
                + "Regler über alles, was sich bewegt: er verkürzt die Schwingungsdauer der "
                + "Dünung, verstärkt ihr winziges Driften und skaliert das Hin- und Herwiegen "
                + "beider feinen Wellenlagen. DIE VOREINSTELLUNG IST 0,12, und das ist eine "
                + "Neueichung und kein Geschmack. Das Urteil zu ModBuild 164 lautete: 'Es fließt "
                + "noch viel zu schnell! Das ist kein Fluss sondern soll eher eine Pfütze "
                + "stehendes Wasser simulieren mit nur minimal Bewegungen.' Bei der alten "
                + "Voreinstellung lösten sich die feinen Wellen zu 0,25 bis 0,50 Metern pro "
                + "Sekunde auf — über eine Kachel von 1,7 m ist das eine sichtbare Strömung. "
                + "Jetzt sind es höchstens etwa 7 cm pro Sekunde, und drei Viertel davon sind ein "
                + "WIEGEN, das umkehrt und sich zu null aufhebt, denn stehendes Wasser hat "
                + "überhaupt keine Strömung. Bei 1,0 schwingt die Dünung so schnell wie echtes "
                + "Wasser dieser Wellenlänge (etwa einmal pro Sekunde); die 0,12 sind ein "
                + "Zwölftel davon. Das Log schreibt die aufgelösten Raten mit, die vier "
                + "Schwingungsdauern der Dünung in Sekunden und wie weit das Muster in einer "
                + "Minute tatsächlich wandert — 'zu schnell' ist also eine Zahl und keine "
                + "Diskussion. 0 friert die Fläche ein, ohne das Relief zu verlieren.",
            ["Water/Shimmer"] =
                ...
                + "zuwendet: jedes Glanzlicht wird auf einem Wellenkamm geboren, wandert mit ihm "
                + "und zerbricht dort, wo Dünung und feine Wellen nicht zusammenpassen. Sie "
                + "bewegen sich NICHT mit deinem Kopf — genau das ist der Unterschied zu der "
                + "Spiegelung, um die es in der ursprünglichen Meldung ging. NEU GEEICHT VON "
                + "0,35: das Glanzlicht war eine enge Spitze und wurde zusätzlich auf die "
                + "DECKKRAFT addiert, ein Wellenkamm wurde also hell und undurchsichtig "
                + "zugleich — das ist bauartbedingt ein weißer Streifen. Jetzt ist es ein "
                + "breiter, matter Schimmer, gemessen gegen die Helligkeit der ruhigen Fläche "
                + "selbst (stilles Wasser glänzt also exakt gar nicht), und er rührt die "
                + "Deckkraft nicht mehr an. FÜR MODBUILD 165 ERNEUT NEU GEEICHT, von 0,10 auf "
                + "0,05: die stehende Vorgabe lautet 'deutlich ruhiger und eher dezent', und eine "
                + "Pfütze in einer Gruft funkelt nicht. 0 nimmt die Glanzlichter heraus und lässt "
                + "die Wellenschattierung stehen. Höher, wenn der Tümpel tot wirkt; niedriger, "
                + "wenn das Funkeln unruhig ist oder kribbelt.",
            ["Water/WaveScale"] =
                "Wie GROSS die Wellen sind, als Vielfaches der ausgelieferten Größe — ein "
                + "einziger Regler über die Wellenlänge der Dünung UND die beiden feinen "
                + "Wellenlagen, damit die Fläche als Ganzes mitwächst und nicht in eine große "
                + "Welle mit falsch skalierter Struktur zerfällt. 1,0 macht die LÄNGSTE der vier "
                + "Dünungskomponenten 2,4 m lang — länger als jede Seite eines Wasserfeldes "
                + "(1,73 x 2,0 m), und das ist Absicht: eine Komponente, die kürzer ist als eine "
                + "Kachel, ist genau der Grund, warum die Fläche in ModBuild 164 auf jedem Feld "
                + "gleich aussah. Höher für eine längere, trägere Dünung; niedriger für "
                + "kabbeligeres Wasser — aber die Zahl LATTICE MISMATCH im Log sagt, ob eine "
                + "Einstellung wieder in die Wiederholung Kachel für Kachel zurückgelaufen ist. "
                + "Die Geschwindigkeit ändert sich dadurch nicht direkt: die Schwingungsdauern "
                + "wachsen mit der Wurzel der Wellenlänge, eine längere Welle braucht also "
                + "entsprechend länger und das Wasser behält seinen Charakter. Das Log schreibt "
                + "die aufgelösten Wellenlängen in Metern mit.",
            ["Water/SwellHeight"] =
                "Wie HOCH die Dünung das Wasser anhebt, als Bruchteil der Breite eines einzelnen "
                + "Wasserfeldes — das 2,6 m breite Feld aus dem Bericht bekommt bei der "
                + "Voreinstellung also 3,6 cm, und ein Diorama in anderem Maßstab bekommt eine "
                + "Welle, die genauso aussieht, statt einer unsichtbaren oder einer riesigen. DAS "
                + "IST ECHTE GEOMETRIE, keine Schattierung: die Grafikkarte unterteilt den Film "
                + "im laufenden Bild, und die dabei entstehenden Punkte bewegen sich wirklich. "
                + "Genau das lässt die Fläche im flachen Blickwinkel dreidimensional wirken, "
                + "statt wie Streifen, die auf eine ebene Platte gemalt sind. NEU GEEICHT VON "
                + "0,045, was direkt an die Obergrenze von 6 cm stieß und über die damalige "
                + "Wellenlänge von 1,1 m eine Wellenflanke von 19 Grad ergab. Die vier "
                + "Komponenten zusammen kommen jetzt auf 8,6 Grad — weniger als die Hälfte der "
                + "Steilheit, also 'deutlich ruhiger und eher dezent' als Winkel ausgedrückt, und "
                + "trotzdem ein Relief, das man an der Wasserlinie deutlich sieht. Die Welle ist "
                + "eine Funktion von Ort und Zeit allein, sie ist also in beiden Augen identisch. "
                + "0 macht den Film wieder flach — das ist zugleich der direkte Vergleich, ob das "
                + "Relief seine Dreiecke wert ist. Die Obergrenze ergibt sich aus den 9 cm, die "
                + "zwischen dem Wasser und dem Beckenbett darunter gemessen wurden: ein Wellental "
                + "darf niemals durch den eigenen Beckenboden stoßen.",
            ["Water/DebugPaint"] =
