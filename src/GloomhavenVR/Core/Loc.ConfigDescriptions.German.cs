    ...
    private static Dictionary<string, string> BuildGerman() =>
        new(784, StringComparer.Ordinal)
        {
            ...
            ["Sky/Style"] =
                ...
                + "Inhalten des Mods im malerischen Stil des Spiels — es wird kein Raum "
                + "aus den Spiel-Assets erzeugt). Default = der eigene animierte Himmel "
                + "des Spiels, exakt wie bisher. Cellar = eine stimmungsvolle, "
                + "kerzenbeleuchtete Keller-Atmosphäre mit schwebenden Staubpartikeln; "
                + "SwampNight = eine mondbeschienene Sumpfnacht unter einer Sternenkuppel "
                + "mit Sternschnuppen, Bodennebel und Glühwürmchen. Eine andere Wahl als "
                + "Default blendet im Szenario die Himmelskugel des Spiels aus und legt "
                + "die Atmosphäre um deinen Standpunkt, zu deiner Blickrichtung gedreht. "
                + "Sie bleibt in jeder Zoomstufe eine FERNE, RUHIGE Umgebung — das Zoomen "
                + "des Bretts bewegt sie nie, bewegt dich nie und verschiebt nie das "
                + "Brett. Stick-Flug, Drehen, das Welt-Greifen und körperliches Gehen "
                + "bewegen dich alle durch sie hindurch. Das Einzige, was sie neu um dich "
                + "herum setzt, ist die Recenter-Geste (B+Y) — dieselbe Geste, die auch "
                + "dich neu zentriert — oder ein erneutes Auswählen des Stils. Sie kann "
                + "den Laser niemals abfangen (keine Collider, nur Mod-Layer). Wirkt "
                + "sofort aus dem VR-Menü, greift sobald ein Szenario läuft. MIXED "
                + "REALITY GEWINNT IMMER: solange MR an ist, ist jeder Himmel und jede "
                + "Umgebung aus, damit der Chroma-Key dein Zimmer zeigen kann; die Wahl "
