...
internal static partial class Loc
{
    private static Dictionary<string, string> BuildGerman() =>
        new(784, StringComparer.Ordinal)
        {
            ...
            // ---- [Sky] ----
            ["Sky/Style"] =
                "Welche Umgebung den Szenario-Tisch umgibt (Nutzer-Entscheid 2026-08-12: die "
                + "Umgebung rendert NUR im Szenario, so wie es die originale Standard-Umgebung "
                + "auch macht — niemals im Menü; und sie nutzt die ECHTEN, fertig gebauten "
                + "Level-Karten des Spiels, keine zusammengewürfelten Einzelteile). Default = "
                + "der eigene animierte Himmel des Spiels, exakt wie bisher. Cellar = eine der "
                + "fertigen Mehrraum-Dungeonkarten des Spiels als kerzenbeleuchteter "
                + "Steinkeller, plus gebündelte Staubpartikel; SwampNight = eine fertige "
                + "Mehrraum-Karte als mondbeschienener Sumpf unter einer gebündelten "
                ...
                + "unter der Welt, jenseits der Sichtweite der Kamera — ein paar Sekunden; die "
                + "Effekte erscheinen sofort) und stellt sie dann als ECHTEN ORT IN DER WELT "
                + "auf — der Hauptraum etwa 10 m groß (die ganze Karte bis 24 m), am Boden "
                + "ausgerichtet, zu deiner Blickrichtung gedreht und MITTIG AUF DEM SPIELFELD: "
                + "das Tisch-Diorama sitzt in der Mitte des Hauptraums (Nutzer-Entscheid "
                + "2026-08-12), die übrigen Räume liegen begehbar um dich herum und der "
                + "Szenario-Tisch bleibt unverändert. Stick-Flug, Drehen, das Welt-Greifen und "
                + "körperliches Gehen bewegen dich alle durch den Raum; der Welt-Zoom skaliert "
                + "nur das Brett, nie den Raum; die Recenter-Geste (B+Y) setzt den Raum wieder "
                + "um dich herum, und ein erneutes Auswählen des Stils baut ihn an deiner "
                + "aktuellen Position neu. Er kann den Laser niemals abfangen (keine Collider, "
                + "nur Mod-Layer). Wirkt sofort aus dem VR-Menü, greift sobald ein Szenario "
                + "läuft. MIXED REALITY GEWINNT IMMER: solange MR an ist, ist jeder Himmel und "
                + "jede Umgebung aus, damit der Chroma-Key dein Zimmer zeigen kann; die Wahl "
                + "greift wieder, sobald MR ausgeht. Werte aus den alten Panorama-Builds "
                + "(Night/Sunset) gibt es nicht mehr, sie fallen auf Default zurück. Rein lokale "
                + "Darstellung, wird nie mit Mitspielern synchronisiert.",
            // ---- [Stereo] ----
            ["Stereo/RenderMode"] =
