    ...
    private static Dictionary<string, string> BuildGerman() =>
        new(784, StringComparer.Ordinal)
        {
            ...
            ["Sky/Style"] =
                ...
                + "Standard-Umgebung auch macht — niemals im Menü; sie nutzt EINEN ECHTEN "
                + "einzelnen Raum aus den Szenario-Karten des Spiels, nie ein ganzes Level "
                + "mit allen Räumen; und sie ist LEBENSGROSS — der ganze Sinn ist, IN der "
                + "Umgebung zu sein, nicht eine Miniatur neben dem Brett anzuschauen). "
                + "Default = der eigene animierte Himmel des Spiels, exakt wie bisher. "
                + "Cellar = ein fertiger Szenario-Raum als kerzenbeleuchteter Steinkeller, "
                + "plus gebündelte Staubpartikel; SwampNight = ein fertiger Szenario-Raum "
                + "als mondbeschienener Sumpf unter einer gebündelten Sternenkuppel mit "
                + "Sternschnuppen, Bodennebel und Glühwürmchen. Der Raum behält seine vom "
                + "Spiel vorgegebene Ausstattung UND sein Leben: Fackelflammen, Staub, "
                + "Tropfen, animierte Requisiten und die flackernden Lichter des Raums "
                + "laufen weiter (nur nicht gesetzte Stil-Achsen werden aufgefüllt; der "
                + "Sumpf bekommt immer den Mondlicht-Ton, damit die Sternenkuppel zur Nacht "
                + "passt), und sein Boden ist garantiert geschlossen — fehlende Bodenstücke "
                + "werden aus der Boden-Grafik des Spiels geheilt oder mit einer "
                + "Nachbarkachel geflickt, nie als Loch gelassen. Eine andere Wahl als "
                + "Default blendet im Szenario die Himmelskugel des Spiels aus, lässt die "
                + "Apparance-Engine des Spiels den Raum komplett außer Sicht bauen — ein "
                + "paar Sekunden, die Effekte erscheinen sofort — und setzt ihn dann "
                + "LEBENSGROSS UM DICH HERUM: rund 11 m breit in deiner aktuellen "
                + "Zoomstufe, zentriert auf deinen Standpunkt, sein Boden auf dem echten "
                + "Boden unter deinen Füßen, zu deiner Blickrichtung gedreht. Der "
                + "Szenario-Tisch steht einfach darin wie ein Diorama in einem Zimmer. "
                + "Während du zoomst, bleiben Brett und Raum starr verbunden (nur die "
                + "wahrgenommene Größe ändert sich — das Brett kann nie mitten in der Geste "
                + "unter den Boden sinken); kommt ein Zoom WEIT außerhalb der Größe zur "
                + "Ruhe, bei der der Raum gesetzt wurde (etwa Faktor 1,4 in beide "
                + "Richtungen), setzt sich der Raum einmal in der neuen Größe um dich herum "
                + "neu — derselbe Sprung wie bei der Recenter-Geste (B+Y), die ebenfalls "
                + "alles neu um dich setzt, genau wie ein erneutes Auswählen des Stils. "
                + "Sternenkuppel, Mond und Staubpartikel bleiben in jeder Zoomstufe ein "
                + "FERNER HIMMEL. Stick-Flug, Drehen, das Welt-Greifen und körperliches "
                + "Gehen bewegen dich alle durch den Raum. Er kann den Laser niemals "
                + "abfangen (keine Collider, nur Mod-Layer). Wirkt sofort aus dem VR-Menü, "
                + "greift sobald ein Szenario läuft. MIXED REALITY GEWINNT IMMER: solange "
                + "MR an ist, ist jeder Himmel und jede Umgebung aus, damit der Chroma-Key "
                + "dein Zimmer zeigen kann; die Wahl greift wieder, sobald MR ausgeht. "
                + "Werte aus den alten Panorama-Builds (Night/Sunset) gibt es nicht mehr, "
                + "sie fallen auf Default zurück. Rein lokale Darstellung, wird nie mit "
