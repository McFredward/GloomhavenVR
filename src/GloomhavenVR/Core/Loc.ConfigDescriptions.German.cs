...
internal static partial class Loc
{
    private static Dictionary<string, string> BuildGerman() =>
        new(784, StringComparer.Ordinal)
        {
            ...
            // ---- [Sky] ----
            ["Sky/Style"] =
                "Welche Umgebung den Szenario-Tisch umgibt (Nutzer-Entscheide 2026-08-12: die "
                + "Umgebung rendert NUR im Szenario, so wie es die originale Standard-Umgebung "
                + "auch macht — niemals im Menü; sie nutzt ECHTE Szenario-Karten des Spiels, "
                + "keine zusammengewürfelten Einzelteile; und Position wie Höhe des "
                + "Spielbretts IM Raum sind FIX). Default = der eigene animierte Himmel des "
                + "Spiels, exakt wie bisher. Cellar = eine der fertigen "
                + "Mehrraum-Szenariokarten des Spiels als kerzenbeleuchteter Steinkeller, "
                + "plus gebündelte Staubpartikel; SwampNight = eine fertige "
                + "Mehrraum-Szenariokarte als mondbeschienener Sumpf unter einer gebündelten "
                + "Sternenkuppel mit Sternschnuppen, Bodennebel und Glühwürmchen. Die Räume "
                + "behalten ihre vom Spiel vorgegebene Ausstattung und Beleuchtung; nur nicht "
                + "gesetzte Stil-Achsen werden aufgefüllt (der Sumpf bekommt immer den "
                + "Mondlicht-Ton, damit die Sternenkuppel zur Nacht passt). Eine andere Wahl "
                + "als Default blendet im Szenario die Himmelskugel des Spiels aus, lässt die "
                + "Apparance-Engine des Spiels die Karte komplett außer Sicht bauen (tief "
                + "unter der Welt, jenseits der Sichtweite der Kamera — ein paar Sekunden; "
                + "die Effekte erscheinen sofort) und stellt sie dann als festen Raum UM DAS "
                + "SPIELBRETT auf: der Hauptraum ist etwa 2,75-mal so groß wie das Spielfeld, "
                + "das Brett sitzt in der Mitte des Hauptraums und ruht mit seiner Unterseite "
                + "auf dem Raumboden wie ein Modell auf einem Tisch, zu deiner Blickrichtung "
                + "gedreht — und dieses Verhältnis ändert sich nie: Zoomen skaliert Brett und "
                + "Raum gemeinsam (nur die wahrgenommene Größe ändert sich, genau wie bei der "
                + "originalen Diorama-Umgebung des Spiels), das Brett kann also nie mehr "
                + "unter den Boden sinken oder durch den Raum wandern. Sternenkuppel, Mond "
                + "und Staubpartikel bleiben dagegen in jeder Zoomstufe ein FERNER HIMMEL. "
                + "Stick-Flug, Drehen, das Welt-Greifen und körperliches Gehen bewegen dich "
                + "alle durch den Raum; die Recenter-Geste (B+Y) setzt alles wieder um dich "
                + "herum, und ein erneutes Auswählen des Stils baut es an deiner aktuellen "
                + "Position neu. Er kann den Laser niemals abfangen (keine Collider, nur "
                ...
                + "und jede Umgebung aus, damit der Chroma-Key dein Zimmer zeigen kann; die "
                + "Wahl greift wieder, sobald MR ausgeht. Werte aus den alten Panorama-Builds "
                + "(Night/Sunset) gibt es nicht mehr, sie fallen auf Default zurück. Rein "
                + "lokale Darstellung, wird nie mit Mitspielern synchronisiert.",
            // ---- [Stereo] ----
            ["Stereo/RenderMode"] =
