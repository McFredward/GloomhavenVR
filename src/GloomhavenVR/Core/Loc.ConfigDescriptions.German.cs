    ...
    private static Dictionary<string, string> BuildGerman() =>
        new(784, StringComparer.Ordinal)
        {
            ...
            ["Water/OwnSurface"] =
                ...
                + "DES MODS IST: die Wellen-Normalkarte des Spiels mit den beiden Kachelungen, die "
                + "das Spiel selbst gestaltet hat, an FESTEN Stellen der Welt abgetastet und "
                + "zwischen drei festen Mustern übergeblendet, die an Ort und Stelle auf- und "
                + "abklingen. Das Funkeln entsteht aus der bewegten Oberfläche unter einer FESTEN "
                + "Lichtrichtung, niemals aus der Blickrichtung: ein Glanzlicht, das aus der "
                + "Position deines Auges berechnet wird, wandert beim Kopfdrehen über die Fläche "
                + "(genau das war der ursprüngliche Fehler) und sieht in jedem Auge anders aus. "
                + "UND DIE FLÄCHE HAT WIRKLICH RELIEF: die Grafikkarte unterteilt jedes "
                ...
                + "weiße Streifen auf einer flachen Oberfläche'. UND NICHTS DARAUF FLIESST, IN "
                + "KEINE RICHTUNG, NIEMALS: die Wellen heben und senken sich GENAU DORT, WO SIE "
                + "SIND — sechs davon, mit Raten ohne gemeinsames Maß, sodass es keinen Rhythmus "
                + "zu bemerken gibt — und ein sehr langsames Aufblühen lässt alle paar Minuten "
                + "einen anderen Teil des Tümpels fast still werden. Genau das bedeutet 'es ist "
                + "kein Fluss sondern eine Pfütze', und genau deshalb passiert auf jedem Feld "
                + "etwas anderes. Feineinstellung über [Water] SwellHeight (wie hoch), [Water] "
                + "WaveScale (wie groß), [Water] RippleSpeed (wie langsam es sich verändert) und "
            ...
            ["Water/RippleSpeed"] =
                ...
                + "also gar nichts schneller fließen lassen: er ist die eine Uhr über alle "
                + "Zyklen. Er teilt die Schwingungsdauer der Dünung, und die drei Überblendzyklen "
            ...
            ["Water/Shimmer"] =
                ...
                + "Pfütze in einer Gruft funkelt nicht. FÜR MODBUILD 166 ERNEUT AUF 0,03, "
                + "zusammen mit allem anderen auf der Fläche: ein Glanzlicht ist eine Änderung "
                + "der Helligkeit, und in dieser Runde geht es genau darum, dass sich die Fläche "
                + "weniger verändert. 0 nimmt die Glanzlichter heraus und lässt "
            ...
            ["Water/WaveScale"] =
                ...
                + "alle sechs aufgelösten Wellenlängen in Metern mit.",
            ["Water/SwellHeight"] =
                ...
                + "Voreinstellung also 1,3 cm, und ein Diorama in anderem Maßstab bekommt eine "
                ...
                + "statt wie Streifen, die auf eine ebene Platte gemalt sind. ZWEIMAL NEU "
                + "GEEICHT: ModBuild 164 lief mit 0,045, was direkt an die Obergrenze von 6 cm "
                + "stieß und über die damalige Wellenlänge von 1,1 m eine Wellenflanke von 19 "
                + "Grad ergab; ModBuild 165 brachte das mit 0,014 auf 8,6 Grad und galt immer "
                + "noch als 'viel zu hektisch'. Die sechs Komponenten zusammen kommen jetzt auf "
                + "2,9 Grad — noch einmal ein Drittel der Steilheit — und genau das heißt 'so gut "
                + "wie KEIN fließen ... sehr dezent' als Winkel ausgedrückt; trotzdem bleibt ein "
                + "Relief, das man an der Wasserlinie sieht, wo über die Flachheit geurteilt "
                + "wurde. Die Welle ist eine Funktion von Ort und Zeit allein, sie ist also in "
                + "beiden Augen identisch, und sie wandert nicht: die Kämme heben und senken sich "
                + "dort, wo sie sind. 0 macht den Film wieder flach — das ist zugleich der "
                + "direkte Vergleich, ob das Relief seine Dreiecke wert ist. Die Obergrenze "
