    ...
    private static Dictionary<string, string> BuildGerman() =>
        new(784, StringComparer.Ordinal)
        {
            ...
            ["Water/OwnSurface"] =
                ...
                + "aus. Feineinstellung über [Water] RippleSpeed und [Water] Shimmer. Beckenbett "
            ...
            ["Water/RippleSpeed"] =
                "Wie schnell der Wasserfilm des Mods fließt, als Vielfaches dessen, was das Spiel "
                + "gestaltet hat. 1,0 ist die gestaltete Geschwindigkeit und die Voreinstellung: "
                + "die beiden Scroll-Raten stammen aus _WaterUVAnimSpeedA/B des Wassermaterials "
                + "selbst und werden zur Laufzeit ausgelesen. WARUM ES ÜBERHAUPT EINEN REGLER "
                + "GIBT — alles andere an dieser Fläche lässt sich gegen das Material des Spiels "
                + "prüfen, nur das hier nicht: die Shader des Spiels liegen fertig kompiliert in "
                + "seinen eigenen Bundles, und auf dem Rechner, auf dem der Mod gebaut wird, gibt "
                + "es keine Installation, mit der man sie öffnen könnte. Wie aus diesen Zahlen "
                + "eine Geschwindigkeit wird, ist also erschlossen und nicht abgelesen. Sieht das "
                + "Wasser aus wie ein Förderband, dreh runter; sieht es aus wie ein Foto, dreh "
                + "hoch. 0 friert die Fläche ein, ohne sonst etwas am Aussehen zu ändern.",
            ["Water/Shimmer"] =
                "Wie kräftig die wandernden Glanzlichter auf dem Wasserfilm des Mods sind. Sie "
                + "entstehen dadurch, dass sich die scrollenden Wellen einer FESTEN Lichtrichtung "
                + "zuwenden: jedes Glanzlicht wird auf einem Wellenkamm geboren, wandert mit ihm "
                + "und zerbricht dort, wo die beiden Wellenlagen aneinander vorbeiziehen. Sie "
                + "bewegen sich NICHT mit deinem Kopf — genau das ist der Unterschied zu der "
                + "Spiegelung, um die es in der ursprünglichen Meldung ging. 0 nimmt die "
                + "Glanzlichter heraus und lässt die Wellenschattierung stehen; das ist das "
                + "Nächste, was diese Fläche noch an der flachen Version aus ModBuild 162 ist. "
                + "Höher, wenn der Tümpel tot wirkt; niedriger, wenn das Funkeln unruhig ist oder "
                + "kribbelt.",
            ["Water/DebugPaint"] =
