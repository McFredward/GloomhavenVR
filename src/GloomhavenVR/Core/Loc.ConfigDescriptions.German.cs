    ...
    private static Dictionary<string, string> BuildGerman() =>
        new(784, StringComparer.Ordinal)
        {
            ...
            ["Sky/Style"] =
                ...
                + "auch macht — niemals im Menü; und sie wird aus den Level-Assets des Spiels "
                + "selbst gebaut). Default = der eigene animierte Himmel des Spiels, exakt wie "
                + "bisher. Cellar = ein kerzenbeleuchteter Steinkeller, generiert aus der "
                + "Dungeon-Grafik des Spiels, plus gebündelte Staubpartikel; SwampNight = ein "
                + "mondbeschienener Sumpf aus der Wald-Grafik des Spiels unter einer gebündelten "
                + "Sternenkuppel mit Sternschnuppen, Bodennebel und Glühwürmchen. Eine andere "
                + "Wahl als Default blendet im Szenario die Himmelskugel des Spiels aus, lässt "
                + "die Apparance-Engine des Spiels den Raum abseits des Blickfelds bauen (ein "
                + "paar Sekunden; die Effekte erscheinen sofort) und stellt ihn dann als ECHTEN "
                + "ORT IN DER WELT auf — etwa 9 m groß, am Boden ausgerichtet, zu deiner "
                + "Blickrichtung gedreht und MITTIG AUF DEM SPIELFELD: das Tisch-Diorama sitzt "
                + "exakt in der Mitte des Raums (Nutzer-Entscheid 2026-08-12), der Raum umgibt "
                + "dich und der Szenario-Tisch bleibt unverändert. Stick-Flug, Drehen, das Welt-Greifen und "
