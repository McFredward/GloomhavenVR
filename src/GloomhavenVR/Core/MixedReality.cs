...
internal static class MixedReality
{
    ...
    private const float RimInsetFallback = 0.03f;
    ...
    private const float RimTopClearanceFallback = 0.025f;
    ...
    internal static void Bind()
    {
        ...
        // MERGE NOTE (round 15): these two keys use LOCAL fallback constants because this branch
        // does not own Defaults/Loc — swap RimInsetFallback → Defaults.UnseenRimInset and
        // RimTopClearanceFallback → Defaults.UnseenRimTopClearance when the Defaults/Loc entries
        // reported with this change land. The bound key names and semantics are final.
        UnseenRimInset = _file.Bind("MixedReality", "UnseenRimInset", RimInsetFallback,
        ...
        UnseenRimTopClearance = _file.Bind("MixedReality", "UnseenRimTopClearance", RimTopClearanceFallback,
    ...
    private static void ForceUnseenOpaque()
    {
        ...
        if (UnseenUnderlays.Count != _loggedRimCount)
        {
            ...
            VRLog.Info("Core", $"MR: unseen RIM CURTAIN — {rimTotal} mod-built dark prism(s) built " +
                               ...
                               $"{(_appliedRimInset > 0f ? _appliedRimInset : RimInsetFallback):0.###} wu " +
                               "INSIDE the authored side faces, from mesh-top − " +
                               $"{Mathf.Max(_appliedRimClearance > 0f ? _appliedRimClearance : RimTopClearanceFallback, (_appliedWaferDrop >= 0f ? _appliedWaferDrop : Defaults.UnseenWaferDrop) + 0.005f):0.###} wu " +
    ...
    private static void BuildUnseenUnderlay(MeshRenderer source, Material[] mats)
    {
        ...
        float rimInset = _appliedRimInset > 0f ? _appliedRimInset : RimInsetFallback;
        float rimClear = _appliedRimClearance > 0f ? _appliedRimClearance : RimTopClearanceFallback;
