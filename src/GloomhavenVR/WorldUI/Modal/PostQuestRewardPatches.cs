...
{
    private static void Prefix(CampaignRewardsManager __instance) =>
        DispatchGuard.Run(nameof(PostQuestCampaignRewardManagerPatch), () => PostQuestRewardSync.CampaignStarting(__instance));
}
...
internal static class PostQuestCampaignRewardShowPatch
{
    private static void Prefix(UICampaignRewardWindow __instance, List<Reward> rewards) =>
        DispatchGuard.Run(nameof(PostQuestCampaignRewardShowPatch), () => PostQuestRewardSync.CampaignShowing(__instance, rewards));
}
...
internal static class PostQuestCampaignRewardContinuePatch
{
    ...
    {
        object? opening = null;
        bool run = DispatchGuard.Run(nameof(PostQuestCampaignRewardContinuePatch),
            () => PostQuestRewardSync.BeforeCampaignContinue(__instance, out opening), onThrow: true);
        __state = opening;
        return run;
    }
    ...
    {
        if (__exception != null) DispatchGuard.Run("PostQuestReward.NativeFailed", () => PostQuestRewardSync.NativeFailed(__state));
        return __exception;
    }
}
...
internal static class PostQuestGuildmasterRewardShowPatch
{
    private static void Prefix(UIGuildmasterAdventureRewardsManager __instance, List<Reward> rewards) =>
        DispatchGuard.Run(nameof(PostQuestGuildmasterRewardShowPatch), () => PostQuestRewardSync.GuildmasterShowing(__instance, rewards));
}
...
internal static class PostQuestGuildmasterRewardContinuePatch
{
    ...
    {
        object? opening = null;
        DispatchGuard.Run(nameof(PostQuestGuildmasterRewardContinuePatch),
            () => opening = PostQuestRewardSync.BeforeGuildmasterHide(__instance));
        __state = opening;
    }
    private static void Postfix(object? __state) => PostQuestRewardSync.NativeSucceeded(__state);
    private static Exception? Finalizer(Exception? __exception, object? __state)
...
internal static class PostQuestRewardIntroductionScopePatch
{
    ...
    {
        uint previous = 0;
        DispatchGuard.Run(nameof(PostQuestRewardIntroductionScopePatch),
            () => previous = PostQuestRewardSync.BeginIntroductionScope(__instance));
        __state = previous;
    }
    private static Exception? Finalizer(Exception? __exception, uint __state)
    {
...
{
    private static void Prefix(UIIntroductionManager.MessageInfo __0) =>
        DispatchGuard.Run(nameof(PostQuestRewardIntroductionMessagePatch), () => PostQuestRewardSync.CaptureIntroduction(__0));
}
...
{
    private static void Prefix(UIIntroductionManager.MessageInfo __0) =>
        DispatchGuard.Run(nameof(PostQuestRewardIntroductionShowPatch), () => PostQuestRewardSync.ShowIntroduction(__0));
}

[HarmonyPatch(typeof(MapChoreographer), "OnDestroy")]
internal static class PostQuestRewardSceneEndPatch
{
    private static void Postfix() => PostQuestRewardSync.ResetAfterNativeSceneEnd();
}
