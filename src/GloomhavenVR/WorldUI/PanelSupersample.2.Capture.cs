...
internal static partial class PanelSupersample
{
    ...
    /// <summary>
    /// <b>THE FIX FOR THE CHARACTER SUB-VIEW (ModBuild 217/218) — remove the game's full-screen
    /// GrabPass blur from a floated window.</b>
    ///
    /// <para><b>WHAT IT WAS.</b> <c>'Blur'</c> under <c>UI Item Confirmation Box</c> carries the
    /// shader <c>Custom/SimpleGrabPassBlur</c> on a rect covering 99 % of the sub-view. A GrabPass
    /// shader GRABS THE FRAMEBUFFER IT IS BEING DRAWN INTO and paints a blurred copy back. Inside a
    /// render-to-texture pass that framebuffer is the capture target MID-RENDER, so the window was
    /// composited with a blurred copy of its own half-finished self; with the supersampler off it
    /// grabs the EYE buffer instead, once per eye per frame, which was the constant flicker. The
    /// over-paint census measured it painting over <b>272 of 764</b> drawing graphics.</para>
    ///
    /// <para><b>WHY IT SURVIVED SIXTEEN BUILDS OF MEASUREMENT.</b> Its inherited alpha is
    /// <c>0.000</c>, so every visibility test in this mod called it hidden and skipped it. A graphic
    /// with a NON-STOCK MATERIAL is not governed by the alpha uGUI writes into its vertex colour: a
    /// custom shader need not honour it, and the grab executes whatever the final blend does with the
    /// result. That is the general lesson and it is worth more than this method.</para>
    ///
    /// <para><b>THE REMEDY IS THE GAME'S OWN</b>, from <c>UIBlurDisabler</c>, which runs it whenever
    /// the player enables Simplified UI — so a blur-free window is a state this UI already ships and
    /// draws correctly in: drop the material, clear the colour, switch the graphic off. The object
    /// stays in place with its layout and hit-testing intact; it is a backdrop and the window has
    /// never needed it drawn. <b>The disabler's OTHER branch is deliberately not reproduced:</b> its
    /// black filter is <c>new Color(17f, 17f, 17f, 85f)</c>, whose components are far outside the
    /// 0..1 range <c>Color</c> takes, so it clamps to OPAQUE WHITE — ModBuild 217 shipped without the
    /// colour lines and the user photographed exactly that white backdrop.</para>
    /// </summary>
