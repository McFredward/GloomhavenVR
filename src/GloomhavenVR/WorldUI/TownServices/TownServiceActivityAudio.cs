...
internal sealed class TownServiceActivityAudio : IDisposable
{
        ...
        in TownActivityVisual shown)
    {
        ...
        try
        {
            ...
            float duration = sound == TownActivitySound.Spell ? 2.8f : sound == TownActivitySound.Coin ? .65f : .65f;
    ...
    private AudioClip? Resolve(TownActivitySound sound, float now)
    {
        ...
        string id = sound == TownActivitySound.Coin ? "PlaySound_ScenarioUIEquipmentToggle_Trinkets"
            : sound == TownActivitySound.Spell ? "PlaySound_ScenarioUIAugmentLight"
