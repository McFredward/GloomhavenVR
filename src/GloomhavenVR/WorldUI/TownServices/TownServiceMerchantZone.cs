...
internal sealed class TownServiceMerchantZone : IDisposable
{
    ...
    internal TownServiceMerchantZone(Transform parent,bool selling,TMP_Text? font)
    {
        ...
        Root.localPosition=new Vector3(selling?.078f:-.078f,.015f,-.20f);
    ...
    internal static GameObject CreateTemplate(TMP_Text? font)
    {
        ...
        var rect=(RectTransform)root.transform;rect.sizeDelta=new Vector2(140f,260f);
        ...
        var text=caption.GetComponent<TextMeshProUGUI>();text.rectTransform.sizeDelta=new Vector2(130f,70f);
        if(font!=null){text.font=font.font;text.fontSharedMaterial=font.fontSharedMaterial;}
        text.fontSize=26f;text.alignment=TextAlignmentOptions.Center;text.color=new Color(.98f,.94f,.8f);text.raycastTarget=false;
