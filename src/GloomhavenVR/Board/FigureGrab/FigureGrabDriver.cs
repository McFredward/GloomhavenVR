...
internal sealed class FigureGrabDriver : MonoBehaviour
{
    ...
    private void LogPropCensus()
    {
        ...
        string line = $"[Props] census: Choreographer.m_ClientObjects held {entries} entr(y/ies), "
            ...
            + $"OVERLAY PULSES alive at once, high-water this scenario: {OverlayPulse.PeakLive} "
            + "(ModBuild 470 — the census ModBuild 455 promised in place of a fix and never wrote). "
            + "The pre-grab highlight is single-winner, so 0, 1 or 2 is CLEAN. THREE OR MORE means "
            + "overlays are outliving the hovers that made them; every OverlayPulse runs off the "
            + "same absolute unscaledTime clock with no per-instance phase, so an accumulating set "
            + "of them IS a board-wide synchronous flash and the white on every trap would be ours. "
            + "A peak of 0 on a session that also holds a 'highlight ENGAGED' line means the "
            + "counter is broken, not that nothing glowed. "
            ...
            + "ModBuild 350 gate (PropLift): every prop it names is one the hand "
            + "passes straight over, with no glow, no ghost and no panel, and the arrow gives the "
            + "TERM that refused it. SINCE ModBuild 450 THERE IS ONLY ONE SUCH TERM for an "
            + "obstacle - the solid-obstacle family test, i.e. a hole in the floor. The "
            + "destructibility term was RETIRED that build, because the user asked for an "
            + "indestructible obstacle (the well: 'einen Brunnen ... nicht zerstoerbares Hindernis') "
            + "to be liftable like every other prop. That is why 'disallowMoveOrDestroy=' is still "
            + "printed on every prop line and is now the column that PROVES the fix rather than the "
            + "one that explained a refusal: a row reading 'disallowMoveOrDestroy=YES ... "
            + "MAYLIFT=yes GRABBABLE=yes' IS the well, lifted, and a build where that flag still "
            + "refuses would show it as an arrow in the REFUSED sentence instead. hasHealth is "
            + "printed on every line because it is the discriminator this gate deliberately does "
            + "NOT use: the ModBuild 350 board had zero props with health, rocks included. And read "
            + "the '[Props] PROP LIFT LEDGER' line beside this one for the same question rolled up "
            + "PER KIND and never sampled, which is the read this capped list cannot give."
