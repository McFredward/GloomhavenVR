...
internal static class NativeBoardCodec
{
    ...
    internal static int Write(NativeBoardState state, byte[] buffer)
    {
        ...
        _ = new NativeBoardState(state.SampleTime, state.InitiativeDepthPixels, state.Generation, state.Elements);
        ...
        for (int offset = 0; offset < total; offset += ChunkBytes)
        {
            ...
            Buffer.BlockCopy(body, offset, buffer, output, count); output += count;
        }
        return output;
    }
    ...
    {
        state = null;
        if (buffer == null || length < 25 || length > buffer.Length || length > MaxSize) return false;
        int at = 0;
        ...
        while (at < length)
        {
            ...
            if (total < 13 || total > BodyMax || offset != filled || offset + count > total
        ...
        try
        {
            ...
            state = new NativeBoardState(time, depth, generation, elements); return true;
        }
