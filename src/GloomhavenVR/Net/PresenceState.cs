...
///            bit4 itemFanHeld, bit5 cardFx, bit6 itemFanLeftHand
...
///
/// The three additive blocks are written and read in FLAG-BIT ORDER (ghost, item fan, card FX).
/// That single rule is what lets independently developed extensions share one packet: each block
/// only has to be appended behind every field a pre-existing reader knows, and the bit order then
/// fixes the layout without any writer/reader having to know about the others.
///
...
internal static class PresenceSerializer
{
    ...
    /// ghost strength 1 + item-fan 1 + card-fx 2 = 36, rounded up to 40 for headroom.</summary>
    ...
    public static int Write(in PresenceState state, byte[] buffer)
    {
        ...
        // Pre-extension readers keep finding handCardCount at the offset they expect, and each
        // block is invisible to a reader whose flags do not include it.
        if (state.GhostHand)
