...
internal static class PresenceSerializer
{
    ...
    /// + 6 (slot-card size: 2 + 4) + 3 (half hover: 2 + 1) + 5 (pile counts: 2 + 3)
    /// + 7 (track hover: 2 + 5) = 454, rounded up to 480 for headroom.
        ...
        if (block)
        {
            ...
            if (extensions)
            {
                    ...
                    && i + 2 + NetProtocol.SlotCardSizeRecordBytes <= buffer.Length)
                {
                    ...
                    // previous build. Appended in id order (11 before 14/15/16).
