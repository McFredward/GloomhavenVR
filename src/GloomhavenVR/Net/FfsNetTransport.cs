...
internal sealed class FfsNetTransport : INetTransport
{
    ...
    public void Send(byte[] payload, int length, object? presentationIdentity = null)
    {
        ...
        try
        {
            ...
            if (type == TownServices.TownServiceCodec.MessageType || type == NetProtocol.MsgExtras || type == NetProtocol.MsgUseBarAnimation || type == NetProtocol.MsgCardPlume || type == NetProtocol.MsgNativeBoard || type == NetProtocol.MsgCardAppearance || type == NetProtocol.MsgNativeDecisionPrompt || type == NetProtocol.MsgItemAppearance)
            {
