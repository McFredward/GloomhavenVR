...
///   if FlagHandStyle: 1 byte hand style (0 Glove/1 Plate/2 Arcane) — ADDITIVE trailing
...
internal static unsafe class AvatarSerializer
{
    ...
    public static int Write(in AvatarState state, byte[] buffer)
    {
        ...
        if (state.HasHeldCard && state.HasHeldMapCard && state.HeldMapKey != 0 && state.HeldMapPoolSeat < state.HeldMapPoolCount)
        {
    ...
    public static bool TryRead(byte[] buffer, int length, out AvatarState state)
    {
        ...
        while (i < length)
        {
            ...
            if (id == NetProtocol.ExtIdHeldMapCard)
            {
                if (size != 9 || state.HasHeldMapCard) return false;
                state.HeldMapKey = ReadU32(buffer, ref i);
                uint pool = ReadU32(buffer, ref i);
                state.HeldMapArcSeat = buffer[i++];
                state.HeldMapPoolSeat = (ushort)pool; state.HeldMapPoolCount = (ushort)(pool >> 16);
