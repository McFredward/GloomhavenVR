...
internal static partial class Defaults
{
    ...
    internal const bool EnvSoundAmbienceBed = true;            // => [EnvSound] AmbienceBed  (ON: it IS the 2026-08-22 request — "ich will für beide eine dezente Hintergrundgeräuschkullise" — and off-by-default would ship the silence he reported. It rides [EnvSound] Enabled, so a player who never chose Cellar or Night forest still hears nothing. ModBuild 222 splits the two rooms' levels after the hardware verdict ("viel zu nervig" in the wood, "kaum was" in the cellar): the cellar's bed is 2.3 dB under the window draught and the wood's is 3.9 dB under that again, both ~45 dB under a game cue at the shipped dial. It is also THE FREE A-B this feature is judged with — off returns the wood's insects to their pre-221 gain and corner, and the build log spells out exactly which emitters it does and does not silence)
    internal const float EnvSoundAmbienceBedGain = 1f;         // => [EnvSound] AmbienceBedGain  (1 = the designed level, derived against the window draught — the one continuous bed the user has judged twice without calling it intrusive. Its own dial rather than riding [EnvSound] Gain because a bed that plays for a whole scenario is the one level a listener is most likely to want to move, and without it the answer would be a rebuild; the key ends in 'Gain' for the ConfigSteps suffix reason above)
