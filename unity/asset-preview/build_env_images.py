...
where a player really stands rather than from a convenient camera.
    ...
    ENV_PREVIEW_OUT=<dir> ENV_PREVIEW_VIEWS=Readme,FireSpill ENV_PREVIEW_NOHAUNT=1 ENV_PREVIEW_NOFIRE=1 \
...
pairs([# FIRE IS SHOT AT A FIRE, NOT AT THE README STATION — the user's verdict was "das Feuer
       # sieht man nicht auf dem Bild", and he was right: ReadmeC looks at the candle table, so
       # under Fire it showed the room BRIGHTENING with no fire in frame, which reads as a light
       # switch. The gated fires are props in fixed places and a frame has to point AT them.
       #
       # AND IT IS FireSpill RATHER THAN FireRoom, on his next report: "beim Feuer-Render sieht
       # man solche Kreise um das Feuer herum — das sehe ich ingame nicht". Those are the
       # Env_GlowSphere shells (EnvGlow.shader) — real scene geometry, an additive sphere whose
       # brightness falls off toward its silhouette. In the headset they read as volumetric haze;
       # on a flat still shot from across the room they read as two hard-edged discs on the
       # masonry. NO CAUSE IS ASSERTED BEYOND THAT, and nothing in the room was changed to suit a
       # picture — the FRAME was changed instead. FireSpill stands close to a burning spill, where
       # the flames and the glowing floor fill the frame and no glow shell is seen against a wall.
       ('env_cellar_FireSpill_ebase.png', 'env_cellar_FireSpill_efireS.png', 'Cellar · Fire',  None),
