...
for name, old, new in [
    ...
    ('retain-burn', 'BurnArtwork.RetireBurnPlayback(BurnArtwork.EffectsOf(FullCard));', ''),
...
PY
for mutation in retain-face drop-identity miss-selected retain-burn unguarded-release; do
    case "$mutation" in
        ...
        retain-burn) expected='Scene release must retire burn protection so native pool resets remain possible' ;;
