...
for mutation in early-terminal passive-tmp-read stale-terminal no-binding-retry material-endpoint offline-retention; do
...
root=pathlib.Path(sys.argv[1]); name,old,new={
    ...
    'offline-retention':('ItemAppearanceSampler.cs','if (!NetAvatarDriver.CanPublishNativePresentation) return;',''),
...
PYTHON
    ...
    case "$mutation" in
        ...
        offline-retention) expected='Offline burns must not retain unsampled item graphs';;
