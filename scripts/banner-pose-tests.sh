...
print('Banner pose: 3 integration bindings and 1 binding negative passed.')
PY
dotnet run --project "$project" --configuration Release
mutation_dir="$(mktemp -d)"
trap 'rm -rf "$mutation_dir"' EXIT
for mutation in native-handoff scale parent release-latch; do
...
changes={
 ...
 'scale': ('banner.localScale = _scale;', 'banner.localScale = Vector3.one;'),
...
PY
    ...
    case "$mutation" in
        native-handoff) expected='native reparent must not retain VR position' ;;
        scale) expected='native reparent must not retain VR scale' ;;
        parent) expected='release preserves parent and sibling chosen by native mode' ;;
        release-latch) expected='repeated release must not overwrite later native writes' ;;
    esac
