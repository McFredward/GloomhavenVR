...
for mutation in inactive-original inactive-unbound inactive-spent-floor live-detach-floor idle-detach-floor cleanup-identity short-rest-preview historical-init historical-rebuild settled-history deferred-reset follower retire-primary retired-follower retargeted-follower recovered-follower implicit-recovery retry-recovery new-episode missing-owner retry-identity terminal-floor activation-floor; do
 python3 - "$work_dir" "$mutation" <<'PY'
from pathlib import Path
import sys
p=Path(sys.argv[1]);s=(p/'production.txt').read_text()
a,b={
'inactive-spent-floor':('&& Playing(fx));', '&& fx.gameObject.activeInHierarchy && Playing(fx));'),
'live-detach-floor':('if (!Playing(fx)) BurnStarts.Remove(fx);', 'BurnStarts.Remove(fx);'),
'idle-detach-floor':('if (!Playing(fx)) BurnStarts.Remove(fx);', ''),
'cleanup-identity':('|| BurnStarts.TryGetValue(fx, out var record) && !ReferenceEquals(record.Card, card)', ''),
...
PY
 ...
 cleanup-identity) expected="A recycled original must not inherit another model's spent history";;
