...
dotnet run --project "$project" --configuration Release \
    ...
    --property:TravelSource="$travel_source"
...
for mutation in missing-curtain-integration unrelated-curtain-escape native-lock-bypass missing-dispatch-admission offline-travel-bypass; do
...
changes = {
    ...
    'unrelated-curtain-escape': (0, 'return ReferenceEquals(candidate, owner) || candidate.transform.IsChildOf(owner.transform);', 'return true;'),
...
if [[ "$mutation" == missing-dispatch-admission ]]; then
    if python3 "$mutation_dir/extract-map-policies.py" "$mutation_dir/Story.fixture" \
        ...
        "$mutation_dir/MapPolicies.fixture" > "$mutation_dir/mutant.log" 2>&1; then
    ...
    --property:TravelSource="$mutation_dir/Travel.fixture" > "$mutation_dir/mutant.log" 2>&1; then
...
case "$mutation" in
    ...
    unrelated-curtain-escape) expected='Unhandled exception. System.InvalidOperationException: Unrelated frozen member must not escape the curtain' ;;
