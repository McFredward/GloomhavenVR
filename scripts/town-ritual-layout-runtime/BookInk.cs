...
internal static class BookInkProof
{
    internal static int Run()
    {
        ...
        foreach(string key in new[]{"temple.level","temple.gold","temple.progress","temple.description"})
        {
            ...
            Check(leaves!=null&&leaves.GetComponent<MeshFilter>()?.sharedMesh.vertexCount>=6000
                &&leaves.GetComponent<Collider>()==null,"native inscription sits over curved blank parchment without a hit blocker");
            Check(leaves!.GetComponent<MeshFilter>().sharedMesh.triangles.Length==36864,
...
internal static class NativeBookProof
{
    internal static int Run()
    {
        ...
        foreach(string key in new[]{"temple.level","temple.gold","temple.progress","temple.description"})
        {
            ...
            Check(blankPages!.GetComponent<MeshFilter>().sharedMesh.triangles.Length==36864,
                "dense blank parchment covers actual raised page details without vertical strips");
